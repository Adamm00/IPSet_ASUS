...
elif [ ! -f "$skynetcfg" ] && grep -E "sh /jffs/scripts/firewall start.* # Skynet" /jffs/scripts/firewall-start | grep -v "usb=" | grep -qvE "^#"; then
	location="/jffs"
	if [ -f "${location}/scripts/ipset.txt" ]; then
		forceupgrade="2"
		if ! echo "$@" | grep -wqE "install"; then
...
Refresh_MWhitelist () {
		if grep -qE "Manual Whitelist.* TYPE=Domain" "$skynetlog"; then
			...
			grep -E "Manual Whitelist.* TYPE=Domain" "$skynetlog" | while IFS= read -r "entry"; do
				url="$(echo "$entry" | awk '{print $9}')"
				echo "${url#*=}" >> /tmp/mwhitelist.list
			done
			sed -i "\\~\\[Manual Whitelist\\] TYPE=Domain~d;" "$skynetlog"
			awk '!x[$0]++' /tmp/mwhitelist.list | while IFS= read -r "website"; do
				for ip in $(Domain_Lookup "$website"); do
					echo "add Skynet-Whitelist $ip comment \"ManualWlistD: $website\"" >> /tmp/mwhitelist2.list && echo "$(date +"%b %d %T") Skynet: [Manual Whitelist] TYPE=Domain SRC=$ip Host=$website " >> "$skynetlog" &
				done
			done
...
Refresh_MBans () {
		if grep -qE "Manual Ban.* TYPE=Domain" "$skynetlog"; then
			...
			grep -E "Manual Ban.* TYPE=Domain" "$skynetlog" | while IFS= read -r "entry"; do
				url="$(echo "$entry" | awk '{print $9}')"
				echo "${url#*=}" >> /tmp/mbans.list
			done
			sed -i "\\~\\[Manual Ban\\] TYPE=Domain~d;" "$skynetlog"
			awk '!x[$0]++' /tmp/mbans.list | while IFS= read -r "website"; do
				for ip in $(Domain_Lookup "$website"); do
					echo "add Skynet-Blacklist $ip comment \"ManualBanD: $website\"" >> /tmp/mbans2.list && echo "$(date +"%b %d %T") Skynet: [Manual Ban] TYPE=Domain SRC=$ip Host=$website " >> "$skynetlog" &
				done
			done
...
Logging () {
		...
		grep -Foc "add Skynet-Black" "$skynetipset" 2> /dev/null > /tmp/counter.txt
		grep -Foc "add Skynet-Block" "$skynetipset" 2> /dev/null >> /tmp/counter.txt
...
case "$1" in
	...
	ban)
		...
		case "$2" in
			...
			country)
				...
				grep -F "/" /tmp/countrylist.txt | sed -n "s/\\r//;/^$/d;/^[0-9,\\.,\\/]*$/s/^/add Skynet-BlockedRanges /p" | sed "s/$/& comment \"Country: $clist\"/" | ipset restore -!
	...
	banmalware)
		...
		if [ -n "$2" ] && [ "$2" != "reset" ]; then
			...
			sed -i '\~New Banmalware Filter~d' "$skynetevents"
			echo "$(date +"%b %d %T") Skynet: [New Banmalware Filter] URL=$listurl " >> "$skynetevents"
		else
			if grep -qF "New Banmalware Filter" "$skynetlog"; then
				listurl="$(grep -F "New Banmalware Filter" "$skynetevents" | sed 's~.*New Banmalware Filter.*URL=~~g')"
				echo "Custom List Detected: $listurl"
			else
	...
	debug)
		case "$2" in
			...
			info)
				...
				if grep -qF "Country:" "$skynetipset"; then echo "Banned Countries; $(grep -m1 -F "Country:" "$skynetipset" | sed 's~.*Country: ~~;s~"~~')"; fi
				...
				iptables -t raw -C PREROUTING -i br0 -m set ! --match-set Skynet-Whitelist dst -m set --match-set Skynet-Master dst -j LOG --log-prefix "[BLOCKED - OUTBOUND] " --log-tcp-sequence --log-tcp-options --log-ip-options 2>/dev/null; then $grn "[Passed]"; elif [ "$debug" = "disabled" ]; then $ylow "[Disabled]"; else $red "[Failed]"; fi
			...
			backup)
				...
				cp "$skynetlog" "$skynetipset"  /tmp/skynetbackup
			...
			search)
				case "$3" in
					...
					ip)
						...
						echo "$(grep -Foc "=$4 " "$skynetlog") Events Total"
						echo
					...
					device)
						...
						echo "$(grep -Eoc -E "OUTBOUND.* SRC=$4 " "$skynetlog") Events Total"
						echo
						$red "Device Name;"
		...
		while true; do
			...
			case "$continue" in
				1)
					...
					rm -rf "/jffs/shared-Skynet-whitelist" "/jffs/shared-Skynet2-whitelist" "/opt/bin/firewall" "$skynetloc" "/jffs/scripts/firewall"
					iptables -t raw -F
					service restart_firewall
					exit 0
				;;
