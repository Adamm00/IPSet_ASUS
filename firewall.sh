		...
		if grep -qE "Manual Whitelist.* TYPE=Domain" "$location/skynet.log"; then
			...
			awk '!x[$0]++' /tmp/mwhitelist.list | while IFS= read -r "website"; do
				for ip in $(Domain_Lookup "$website"); do
					echo "add Whitelist $ip comment \"ManualWlistD: $website\"" >> /tmp/mwhitelist2.list && echo "$(date +"%b %_d %T") Skynet: [Manual Whitelist] TYPE=Domain SRC=$ip Host=$website " >> "${location}/skynet.log" &
				done
			done
...
Refresh_MBans () {
		if grep -qE "Manual Ban.* TYPE=Domain" "$location/skynet.log"; then
			...
			awk '!x[$0]++' /tmp/mbans.list | while IFS= read -r "website"; do
				for ip in $(Domain_Lookup "$website"); do
					echo "add Blacklist $ip comment \"ManualBanD: $website\"" >> /tmp/mbans2.list && echo "$(date +"%b %_d %T") Skynet: [Manual Ban] TYPE=Domain SRC=$ip Host=$website " >> "${location}/skynet.log" &
				done
			done
...
case "$1" in
	...
	ban)
		...
		case "$2" in
			ip)
				...
				ipset -A Blacklist "$3" comment "ManualBan: $desc" && echo "$(date +"%b %_d %T") Skynet: [Manual Ban] TYPE=Single SRC=$3 COMMENT=$desc " >> "${location}/skynet.log"
			;;
			range)
				if ! echo "$3" | Is_Range; then echo "$3 Is Not A Valid Range"; echo; rm -rf /tmp/skynet.lock; exit 2; fi
				if [ "${#4}" -gt "255" ]; then echo "$4 Is Not A Valid Comment. 255 Chars Max"; echo; rm -rf /tmp/skynet.lock; exit 2; fi
				...
				ipset -A BlockedRanges "$3" comment "ManualRBan: $desc" && echo "$(date +"%b %_d %T") Skynet: [Manual Ban] TYPE=Range SRC=$3 COMMENT=$desc " >> "${location}/skynet.log"
			;;
			domain)
				if [ -z "$3" ]; then echo "Domain Field Can't Be Empty - Please Try Again"; echo; rm -rf /tmp/skynet.lock; exit 2; fi
				...
				for ip in $(Domain_Lookup "$3"); do
					echo "Banning $ip"
					ipset -A Blacklist "$ip" comment "ManualBanD: $3" && echo "$(date +"%b %_d %T") Skynet: [Manual Ban] TYPE=Domain SRC=$ip Host=$3 " >> "${location}/skynet.log"
				done
			;;
			country)
	...
	banmalware)
		...
		if [ -n "$2" ] && [ "$2" != "reset" ]; then
			...
			sed -i '\~New Banmalware Filter~d' "${location}/skynet.log"
			echo "$(date +"%b %_d %T") Skynet: [New Banmalware Filter] URL=$listurl " >> "${location}/skynet.log"
		else
	...
	whitelist)
		...
		case "$2" in
			ip|range)
				...
				ipset -A Whitelist "$3" comment "ManualWlist: $desc" && sed -i "\\~$3 ~d" "${location}/skynet.log" && echo "$(date +"%b %_d %T") Skynet: [Manual Whitelist] TYPE=Single SRC=$3 COMMENT=$desc " >> "${location}/skynet.log"
			...
			domain)
				...
				for ip in $(Domain_Lookup "$3"); do
					...
					ipset -A Whitelist "$ip" comment "ManualWlistD: $3" && sed -i "\\~$ip ~d" "${location}/skynet.log" && echo "$(date +"%b %_d %T") Skynet: [Manual Whitelist] TYPE=Domain SRC=$ip Host=$3 " >> "${location}/skynet.log"
					ipset -q -D Blacklist "$ip"
				done
			;;
			port)
				...
				grep -F "NEW" "${location}/skynet.log" | grep -F "DPT=$3 " | grep -oE 'SRC=[0-9,\.]* ' | cut -c 5- | while IFS= read -r "ip"; do
					echo "Whitelisting $ip"
					ipset -A Whitelist "$ip" comment "ManualWlist: Port $3 Traffic" && sed -i "\\~$ip ~d" "${location}/skynet.log" && echo "$(date +"%b %_d %T") Skynet: [Manual Whitelist] TYPE=Port SRC=$ip Host=$3 " >> "${location}/skynet.log"
					ipset -q -D Blacklist "$ip"
				done
			;;
			vpn)
