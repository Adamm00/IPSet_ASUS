...
Logging () {
		...
		if iptables -nvL -t raw | grep -Fv "LOG" | grep -qF "Skynet src"; then
	...
	debug)
		case "$2" in
			...
			info)
				...
				if [ -f "/tmp/skynet.lock" ]; then if [ ! -d "/proc/$(cat /tmp/skynet.lock)" ]; then $grn "No Lock File Found"; else $red "Lock File Detected (pid=$(cat /tmp/skynet.lock))"; fi; else $grn "No Lock File Found"; fi
				...
				if iptables -nL -t raw | grep -v "LOG" | grep -qF "match-set Skynet "; then $grn "Skynet IPTable Detected"; else $red "Skynet IPTable Not Detected"; fi
